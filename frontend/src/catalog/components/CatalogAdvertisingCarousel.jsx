...
import "../../main/styles/HomePage1.css";
...
const NovelCard = ({ title, description, image, slug, book_type, adult_content }) => {
  ...
  const handleReadClick = async (e) => {
    e.stopPropagation();
    console.log('CatalogAdvertisingCarousel: клик по кнопке "читати"', { title, slug });
  ...
  const handleCardClick = async () => {
    console.log('CatalogAdvertisingCarousel: клик по карточке', { title, slug });
  ...
  if (!title || !slug) {
    console.warn('CatalogAdvertisingCarousel: отсутствуют обязательные данные:', { title, slug });
    return (
...
const CatalogAdvertisingCarousel = () => {
    ...
    <div className="main-container">
      ...
      <div className="novels-slider-wrapper">
        {isLoading ? (
          ...
          <div className="no-books-message">Немає активних рекламних оголошень для каталогу</div>
        )}
      </div>
...
export default CatalogAdvertisingCarousel;
